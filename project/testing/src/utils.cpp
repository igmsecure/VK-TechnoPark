...
int IO_TEST::switchStreamToFile(const std::string &file, FILE *stream, const std::string &mode) {
    fflush(stream);
    /* Получение дескриптора файла, связанного с исходным стандартным выводом */
    int currentStreamDescriptor = fileno(stream);
    if (currentStreamDescriptor == -1) {
        throw std::runtime_error("fileno");
    }
    /* Получиение копии файлового дескриптора, связанного с исходным потоком */
...
void IO_TEST::switchBackStream(int originalDescriptor, FILE *stream) {
    ...
    fflush(stream);
    /* Получение файлового дескриптора, связанного с текущим потоком*/
    int currentStreamDescriptor = fileno(stream);
    if (currentStreamDescriptor == -1) {
        throw std::runtime_error("fileno");
    }
    /* Закрыть текущий дескриптор, связанный с потоком и
     * связать его с исходным дескриптором */
